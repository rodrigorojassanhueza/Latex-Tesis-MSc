...
\def\documentinterline {1.15}      % Interlineado del documento [factor]
\def\documentparindent {15}        % Tamaño del indentado de párrafos [pt]
\def\documentparskip {12}          % Tamaño adicional entre párrafos (+/-) [pt]
\def\fontdocument {times}          % Tipografía base, ver soportadas en manual
...
\def\apacitestyle {bibliography/apacite_noinitials} % Formato refs. apacite {apa,ieeetr,etc..}
...
\def\natbibrefstyle {natnumur}    % Formato refs. natbib {apa,ieeetr,etc...}
...
\def\defaultimagefolder {assets/figures/template/} % Carpeta raíz de las imágenes
...
\def\tablenotesameline {False}      % Notas en tablas en una sola línea
...
\def\linkcolor {\maintextcolor}    % Color de los links del documento
\def\numcitecolor {\maintextcolor} % Color del n° de las referencias o citas
...
\def\urlcolor {\maintextcolor}     % Color de los enlaces web (\href,\url)
