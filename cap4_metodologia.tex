...
\begin{enumerate}
    ...
    \item \textbf{Ejecución de corridas en OpenQuake:} amenaza determinista (DSHA), amenaza probabilista (PSHA) y riesgo (por escenarios y event-based).
...
\begin{figure}[H]
	...
	\node[box, below=of Input4] (Input5) {
		...
		\bitem Jünemann et al. (2024)\\
		\bitem Cabrera et al. (2024)\\
		\bitem HAZUS
	};
	...
	\node[box, below=0mm of HeadAmenaza, sharp corners=north, yshift=1.5pt] (Calc1) {
		...
		\bitem FSR: $M_w=7.5$; dip 34/60; $z_{\min}$ 0/5 km\\
		\bitem Interplaca: $M_w=9.3$\\
		\bitem Intraplaca: $M_w=8.0$; rotación 0/20/40
	};
...
\begin{itemize}
    \item \textbf{Amenaza determinista (DSHA):} escenarios representativos de FSR, interplaca e intraplaca, con mapas de intensidades para PGA y $SA(1.0)$ en la cuenca.
...
\small
\begin{tabular}{p{3.2cm} p{4.6cm} p{4.2cm} p{3.2cm}}
\hline
...
La traza completa de la FSR se obtuvo de \shortcite{campillay2025san} y se empleó para construir la franja de análisis descrita en la Sección~\ref{sec:meto_buffer}. Las manzanas censales se obtuvieron de la cartografía oficial asociada al Censo 2018 (INE) y se utilizaron como unidad de agregación espacial del riesgo, asociando el inventario de exposición a cada manzana mediante operaciones de intersección espacial.

La caracterización de condiciones de sitio se basó en zonificaciones y modelos de velocidad disponibles para la cuenca de Santiago \shortcite{OrtizWall2022,acevedo2021modelo,Salomon2021,Bustos2023}. En particular, se asignaron a cada sitio de la malla valores de $V_{S30}$ y parámetros vinculados a la profundidad del contraste suelo--roca, específicamente $Z_{1.0}$ y $Z_{2.5}$, manteniendo la consistencia espacial entre capas.
...
La Falla San Ramón (FSR) se incorporó al modelo integrado mediante un conjunto de alternativas geométricas y de recurrencia coherentes con la evidencia sismotectónica disponible. Se consideraron tres configuraciones de traza (PO, PP y PP+RO; Figura~\ref{fig:Traza FSR}), que difieren en la representación de ramales y en la extensión de la traza incluida en la fuente. La magnitud máxima se exploró en el rango $M_w \in [6.8,\,7.5]$.

La recurrencia se parametrizó mediante periodos de retorno entre 6\,500 y 8\,000 años, con incrementos de 500 años. La implementación final adoptó una distribución discreta de magnitud (MFD discreta), asignando a cada magnitud una tasa de ocurrencia consistente con los valores explorados.

\subsubsection{Fuentes de subducción: contribuciones interplaca e intraplaca}
\label{sec:meto_subduccion}

La contribución de subducción se definió a partir de la zonificación de recurrencia para sismicidad interplaca e intraplaca propuesta por Poulos et al.\ (2019) y de la geometría de la interfaz y del slab provista por Slab2 (Hayes, 2018). En el PSHA se incluyeron simultáneamente las siete zonas de recurrencia de Poulos et al.\ (2019). En el DSHA se definieron escenarios representativos consistentes con dicha zonificación: un escenario interplaca basado en la zona~2 completa y un escenario intraplaca construido sobre una porción representativa de la zona intraplaca correspondiente.
...
Para la fuente cortical asociada a la FSR se adoptó una magnitud fija $M_w=7.5$ y se evaluó la sensibilidad a parámetros geométricos mediante la combinación de dos inclinaciones (34° y 60°) y dos profundidades mínimas de ruptura (0 y 5~km), resultando en cuatro configuraciones:
\begin{itemize}
...
La modelación de pérdidas se implementó con tres configuraciones de funciones de fragilidad: Jünemann et al.\ (2024), Cabrera et al.\ (2024) y HAZUS (FEMA, 2020). Los dos modelos nacionales se reportaron tanto de manera individual como de forma conjunta bajo la denominación \textit{modelo nacional} (Jünemann + Cabrera), dado que fueron desarrollados sobre inventarios no superpuestos y, en conjunto, cubren las tipologías presentes en la franja de análisis.
...
\begin{table}[H]
\centering
\caption{Tipologías consideradas y correspondencia con clases de HAZUS (FEMA, 2020) y del modelo nacional (Jünemann et al., 2024; Cabrera et al., 2024).}
...
\vspace{2mm}
\footnotesize
\emph{Nota:} \textsuperscript{J} tipología provista por Jünemann et al.\ (2024);
\textsuperscript{C} tipología provista por Cabrera et al.\ (2024).
\end{table}
...
\caption{Funciones de vulnerabilidad estructural de HAZUS (FEMA, 2020), expresadas como pérdida relativa media en función de $\mathrm{PGA}$.}
...
\caption{Funciones de vulnerabilidad estructural de Jünemann et al.\ (2024), expresadas como pérdida relativa media en función de $\mathrm{PGA}$ y $SA(1.0)$, según la tipología reportada.}
...
La distancia máxima de contribución se definió por tipo de fuente. Para la contribución cortical asociada a la Falla San Ramón (FSR) se utilizó una distancia constante de 300~km. Para subducción, la distancia máxima se especificó como una función escalonada de $M_w$, diferenciando entre sismicidad interplaca e intraplaca/intraslab.

\paragraph{Determinación de la distancia de corte $R_{\mathrm{cut}}$.}
Para justificar y contrastar las distancias máximas adoptadas en subducción, se estimó una \emph{distancia de corte} $R_{\mathrm{cut}}$ para cada magnitud $M_w$ a partir de modelos de predicción del movimiento del suelo. El criterio es simple: para una magnitud dada, $R_{\mathrm{cut}}$ corresponde a la distancia a la ruptura a partir de la cual el nivel de intensidad esperado resulta menor que el umbral mínimo de interés ($0.05\,g$).

En la práctica, para cada $M_w$ y una malla de distancias a la ruptura $R_{rup}$, se evaluaron cinco modelos (MBR17, PSHAB2020, KBCG2020, BCHU18 y AG2020). Con estas predicciones se construyó una envolvente superior incorporando la dispersión aleatoria mediante niveles $k\,\sigma$. Luego, $R_{\mathrm{cut}}$ se tomó como la menor distancia $R_{rup}$ para la cual dicha envolvente cae por debajo de $0.05\,g$ (en esta sección se ilustra el procedimiento con $\mathrm{PGA}$). Para evitar oscilaciones asociadas a discretización y a cambios del modelo dominante, la relación $R_{\mathrm{cut}}(M_w)$ se forzó a ser no decreciente con $M_w$.

Las Figuras~\ref{fig:meto_event_dist_mw_plot_pga} y~\ref{fig:meto_event_dist_mw_plot_sa} muestran $R_{\mathrm{cut}}(M_w)$ para interplaca e intraplaca/intraslab y distintos niveles $k\,\sigma$ (curvas de color). La curva negra corresponde a las distancias máximas por tramos que se adoptaron en los cálculos, incluidas para comparar visualmente el grado de conservadurismo o cercanía de la parametrización final respecto de las estimaciones derivadas de los modelos.
...
\begin{figure}[htbp]
	...
	\begin{minipage}{0.48\textwidth}
		...
		\subcaption{Intraplaca}
	\end{minipage}
	\caption{Distancia de corte $R_{\mathrm{cut}}$ en función de la magnitud $M_w$ para sismicidad de subducción interplaca (a) e intraplaca/intraslab (b), calculada para $SA(1.0)$ con umbral $IM_{\min}=0.05\,g$. Para cada $M_w$, $R_{\mathrm{cut}}$ representa la distancia máxima de contribución obtenida a partir de una envolvente basada en varios modelos de predicción del movimiento del suelo, considerando distintos niveles $k\,\sigma$ (curvas de color). La curva negra corresponde a las restricciones de distancia máxima por magnitud implementadas en OpenQuake (parámetro \texttt{maximum\_distance}), incluidas para comparar los valores adoptados en el cálculo con los estimados mediante los modelos.}
...
La submuestra se generó con semilla fija y se guardó junto con metadatos (parámetros y semilla) para asegurar trazabilidad. Su representatividad se verificó comparando (i) cobertura espacial (mapas y distancia a vecino más cercano) y (ii) distribuciones de $V_{S30}$ y $Z_{1.0}$ (histogramas), además de diagnósticos por grilla con residuales de Pearson y contribuciones a $\chi^2$:
\begin{equation}
...
\begin{equation}
D_{\mathrm{KS}}=\sup_x \left|F_{\mathrm{all}}(x)-F_{\mathrm{sub}}(x)\right|,
\label{eq:ks}
\end{equation}
\begin{itemize}
  \item $F_{\mathrm{all}}(x)$, $F_{\mathrm{sub}}(x)$: funciones de distribución empíricas del conjunto completo y la submuestra.
\end{itemize}
...
\paragraph{Umbrales de excedencia.}
Se consideraron $\mathrm{PGA}=0.2\,g$ y $\mathrm{PGA}=0.4\,g$.

\paragraph{Estadístico espacial y curva $T(u)$.}
Para cada evento $e$ y umbral $y^\ast$, se definió el conteo de excedencias espaciales:
\begin{equation}
...
Para cuantificar la incertidumbre asociada al uso de un número finito de GMFs, se aplicó bootstrap por eventos: para cada $n$ candidato se generaron $B$ réplicas remuestreando $n$ eventos con reemplazo y obteniendo $\widehat{T}^{(b)}_{y^\ast}(k)$. La convergencia se evaluó con el máximo error vertical respecto de $\widehat{T}^{\,\mathrm{ref}}_{y^\ast}(k)$, restringiendo la comparación a $[0.025,\,0.975]$:
\begin{equation}
\Delta^{(b)}_{y^\ast}(n)=
...
La comparación se interpretó verificando si $\Delta_{\mathrm{obs}}(u)$ queda mayoritariamente dentro del intervalo percentil 2.5--97.5 de $\delta(u)$, lo que indica que las discrepancias completo--reducido son del orden de la variabilidad esperable por usar un número finito de GMFs.
...
El número de ramas del árbol lógico controla el costo computacional y, al mismo tiempo, la estabilidad de los resultados. Para fijar un número mínimo de ramas $N$ que mantenga resultados estables, se evaluó la convergencia de una intensidad equivalente $IM^\ast$ asociada a un período de retorno alto ($T_R=10{,}000$ años), usando remuestreo \textit{bootstrap} sobre las realizaciones del árbol lógico.

\paragraph{Probabilidad objetivo.}
Bajo ocurrencia Poisson, la probabilidad de excedencia en el horizonte $T_{\mathrm{inv}}$ asociada al período de retorno objetivo $T_R^\ast$ se calculó como:
\begin{equation}
\mathrm{PoE}_{\mathrm{target}}=1-\exp\!\left(-\frac{T_{\mathrm{inv}}}{T_R^\ast}\right).
\end{equation}
\begin{itemize}
  \item $T_{\mathrm{inv}}$: horizonte temporal usado en la comparación (en este trabajo, $50$ años).
...
\end{equation}
\begin{itemize}
  \item $N_{\mathrm{tot}}$: número total de realizaciones del árbol lógico presentes en el ZIP.
  \item $p$: percentil analizado (se trabajó con tres percentiles: $p16$, $p50$, $p84$).
\end{itemize}

Para que $p16$, $p50$ y $p84$ representen $-1\sigma$, mediana y $+1\sigma$ de una normal estándar truncada en $\pm 3\sigma$, se usaron percentiles normalizados:
\begin{equation}
...
En esta etapa se aplicó muestreo uniforme sobre realizaciones (consistente con lo implementado en el código).
...
Luego, para que un mismo $N$ garantice convergencia simultánea de $p16$, $p50$ y $p84$ en el sitio $s$, se tomó:
\begin{equation}
N^{(\tau)}_{s}=\max_{p\in\{p16,p50,p84\}} N^{(\tau)}_{p,s}.
\end{equation}
\begin{itemize}
...
Para reducir costo, $N$ se exploró primero en una malla gruesa (incrementos de 100 ramas) y, al detectar convergencia al $2\%$, se refinó localmente con paso 1 para obtener el $N$ mínimo consistente con el criterio de estabilidad.
