...
\begin{itemize}
    \item \(IM\): medida de intensidad del movimiento del suelo (p.\ ej., \(PGA\), \(SA(T)\)).
...
Una medida de intensidad (\(IM\), \textit{Intensity Measure}) es una variable escalar que resume la severidad del movimiento del suelo relevante para la respuesta estructural \cite{BakerBradleyStafford2021}. Ejemplos habituales incluyen \(PGA\) y \(SA(T)\).
...
Bajo un supuesto poissoniano para la ocurrencia de excedencias, la probabilidad de exceder \(im\) en un horizonte \(T\) se relaciona con \(\lambda\) mediante \cite{Cornell1968}:
\begin{equation}
P(IM > im \mid T) = 1 - \exp\left[-\lambda(IM > im)\,T\right].
\label{eq:poe_from_lambda}
\end{equation}
donde \(P(IM>im\mid T)\) denota la probabilidad de excedencia del umbral en el periodo \(T\).
...
\begin{enumerate}
    ...
    \item \textbf{GMPEs}: entregan la distribución de \(IM\) condicionada a parámetros de fuente, trayectoria y sitio.
\end{enumerate}
...
La probabilidad condicional de ocurrencia de al menos un evento en el intervalo \(\Delta t\), dado que han transcurrido \(t_p\) años desde el último evento, se expresa como \cite{MatthewsEllsworthReasenberg2002}:
\begin{equation}
P(\Delta t \mid t_p) = \frac{F_{\text{BPT}}(t_p + \Delta t) - F_{\text{BPT}}(t_p)}{1 - F_{\text{BPT}}(t_p)}.
...
\end{equation}
donde \(\varepsilon\) se interpreta típicamente como \(\varepsilon = \frac{\ln(IM)-\mu_{\ln IM}}{\sigma_{\ln IM}}\) \cite{BakerCornell2005}. Esta herramienta permite interpretar qué escenarios controlan un nivel de amenaza dado \cite{BazzurroCornell1999}.
